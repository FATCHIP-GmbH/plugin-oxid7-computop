...
{{ script({ include: oViewConf.getModuleUrl('fatchip_computop_payments','js/creditcard.js'), priority: 30, dynamic: __oxid_include_dynamic }) }}
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        computopForm.addEventListener('submit', async (event) => {
            event.preventDefault();
            try {
                const response = await fetch(computopForm.action, {
                    method: 'POST',
                    body: new FormData(computopForm)
                });

                if (response.ok) {
                    document.getElementById('orderConfirmAgbBottom').submit();
                } else {
                    console.error('Error during the first form submission.');
                }
            } catch (error) {
